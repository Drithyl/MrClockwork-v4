...
class MultiSelect {
  constructor(element, options = {}) {
    ...
    for (const prop in this.selectElement.dataset) {
      if (this.options[prop] !== undefined) {
        if (
          typeof this.options[prop] === "boolean" ||
          this.selectElement.dataset[prop] === "true" ||
          this.selectElement.dataset[prop] === "false"
        ) {
  ...
  _template() {
    ...
    for (const [groupName, items] of Object.entries(groupedData)) {
      if (groupName) {
        ...
        optionsHTML += `<div class="multi-select-group${allGroupSelected ? " multi-select-selected" : ""}" data-group="${this._escapeHTML(groupName)}" role="option" tabindex="-1">`

        if (this.options.groupLevelSelect) {
          optionsHTML += `    <span class="multi-select-option-radio"></span>`;
        }

        optionsHTML += `    <span class="multi-select-option-text">${this._escapeHTML(groupName)}</span>`;
        optionsHTML += `</div>`;
      }
      items.forEach((item) => {
        const isSelected = item.selected;
        const isDisabled = item.disabled;
        optionsHTML += `<div class="multi-select-option${isSelected ? " multi-select-selected" : ""}" data-value="${this._escapeHTML(item.value)}" data-group="${this._escapeHTML(groupName)}" role="option" aria-selected="${isSelected}" tabindex="-1" ${isDisabled ? 'style="opacity: 0.5; pointer-events: none;"' : ""}>`
        optionsHTML += `    <span class="multi-select-option-radio"></span>`;
        optionsHTML += `    <span class="multi-select-option-text">${item.html ? item.html : this._escapeHTML(item.text)}</span>`
        optionsHTML += `</div>`;
      });
    }
    let selectAllHTML = "";
    if (this.options.selectAll) {
      ...
      selectAllHTML += `    <span class="multi-select-option-radio"></span>`;
    ...
    template += `        <span class="multi-select-header-max">${this.options.max ? this.selectedValues.length + "/" + this.options.max : ""}</span>`;
    template += `        <span class="multi-select-header-placeholder">${this._escapeHTML(this.placeholder)}</span>`;
    template += `    </div>`;
    template += `    <div class="multi-select-options" style="${this.options.dropdownWidth ? "width:" + this.options.dropdownWidth + ";" : ""}${this.options.dropdownHeight ? "height:" + this.options.dropdownHeight + ";" : ""}" role="listbox">`;
    template += `        ${this.options.search ? '<input type="text" class="multi-select-search" placeholder="Search..." role="searchbox">' : ""}`;
    template += `        ${selectAllHTML}`;
  ...
  _eventHandlers() {
    ...
    this.element.querySelectorAll(".multi-select-group").forEach((groupEl) => {
      groupEl.onclick = (e) => {
        e.stopPropagation();
        if (this.element.classList.contains("disabled")) return;
        if (this.options.groupLevelSelect === false) return;
        ...
        this.element.querySelectorAll(".multi-select-option").forEach((option) => {
          if (option.dataset.group === groupName && option.style.display !== "none") {
            ...
            if (
              dataItem &&
              !dataItem.disabled &&
              ((!isSelected && !dataItem.selected) || (isSelected && dataItem.selected))
            ) {
              if (
                this.options.max &&
    ...
    if (this.options.search) {
      ...
      search.oninput = () => {
        ...
        this.element.querySelectorAll(".multi-select-group").forEach((group) => {
          if (
            visibleGroups.has(group.dataset.group) ||
            group
    ...
    if (this.selectElement.id) {
      if (this.labelClickHandler) {
        document
          .querySelectorAll(`label[for="${CSS.escape(this.selectElement.id)}"]`)
          .forEach((label) => {
    ...
    headerElement.addEventListener("keydown", (e) => {
      if (["Enter", " ", "ArrowDown", "ArrowUp"].includes(e.key)) {
        ...
        toggleDropdown();
        const firstElement =
          this.element.querySelector('[role="searchbox"]') ||
          this.element.querySelector('[role="option"]');
        ...
        if (["ArrowDown", "ArrowUp"].includes(e.key)) {
          ...
          while (
            ...
            (nextElement.style.display === "none" ||
              ...
              !nextElement.matches('[role="option"],[role="searchbox"]'))
          ) {
  ...
  _updateHeader() {
    this.element
      .querySelectorAll(".multi-select-header-option, .multi-select-header-placeholder")
    ...
    if (this.selectedValues.length > 0) {
      if (this.options.listAll) {
        this.selectedItems.forEach((item) => {
          ...
          this.element.querySelector(".multi-select-header").append(el);
        });
      } else {
        this.element
  ...
  _validate() {
    ...
    if (!isValid) {
      ...
      if (this.selectElement && this.options.required)
        this.selectElement.setCustomValidity("Please fill out this field.");
    } else {
  ...
  _outsideClick(event) {
    ...
    const labelSelector = this.selectElement.id
      ? `label[for="${CSS.escape(this.selectElement.id)}"]`
      : null;
  ...
  get lastSelectedIndex() {
    return this.data.findLastIndex((item) => item.selected === true);
  }
  get data() {
