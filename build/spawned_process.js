...
function SpawnedProcess(exePath, args) {
    ...
    this.onExited = function (doThis) { return _process.on("exit", function (code, signal) {
        if (signal == null)
            doThis(code);
    }); };
    this.onTerminated = function (doThis) { return _process.on("exit", function (code, signal) {
        if (code == null)
            doThis(signal);
    }); };
    this.onStdioExited = function (doThis) { return _process.on("close", function (code, signal) {
        if (signal == null)
            doThis(code);
    }); };
    this.onStdioTerminated = function (doThis) { return _process.on("close", function (code, signal) {
        if (code == null)
            doThis(signal);
    }); };
