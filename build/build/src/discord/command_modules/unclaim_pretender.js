...
var commandData = new CommandData("UNCLAIM_PRETENDER");
module.exports = UnclaimPretenderCommand;
function UnclaimPretenderCommand() {
    var unclaimPretenderCommand = new Command(commandData);
    unclaimPretenderCommand.addBehaviour(_behaviour);
    unclaimPretenderCommand.addRequirements(commandPermissions.assertMemberIsTrusted, commandPermissions.assertCommandIsUsedInGameChannel, commandPermissions.assertGameIsOnline, commandPermissions.assertGameHasNotStarted);
...
function _behaviour(commandContext) {
    ...
    var nameOfNationToBeUnclaimed = commandArguments[0];
    ...
    return gameObject.unclaimPretender(nameOfNationToBeUnclaimed)
        .then(function () { return commandContext.respondToCommand("Pretender was unclaimed."); })
        .catch(function (err) { return commandContext.respondToCommand("Error occurred when unclaiming pretender:\n\n" + err.message); });
}
//# sourceMappingURL=unclaim_pretender.js.map