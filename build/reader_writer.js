...
module.exports.copyDir = function (source, target, deepCopy, extensionFilter, cb) {
    ...
    return new Promise(function (resolve, reject) {
        if (fs.existsSync(source) === false) {
            var err = new Error("The source path " + source + " does not exist.");
            if (typeof cb === "function")
                return cb(err);
            else
                return reject(err);
        ...
        fs.readdir(source, function (err, filenames) {
            ...
            filenames.forEachPromise(function (file, index, next) {
...
module.exports.atomicRmDir = function (target, filter) {
    var renamedFiles = [];
    var untouchedFiles = [];
    var targetName = (target.indexOf("/") === -1) ? target : target.slice(target.lastIndexOf("/") + 1);
    return new Promise(function (resolve, reject) {
        ...
        fs.stat(target, function (err, stats) {
            ...
            fs.mkdir(config.tmpDir + "/" + targetName, function (err) {
                if (err)
                    return reject(err);
                //fetch filenames of the target dir
                fs.readdir(target, function (err, filenames) {
                    if (err)
                        return reject(err);
                    ...
                    for (var i = 0; i < filenames.length; i++) {
                        ...
                        try {
                            var fileStat = fs.statSync(target + "/" + filename);
                    ...
                    renamedFiles.forEachPromise(function (filepaths, index, next) {
                        //unlink renamed file at new tmp path
                        fs.unlink(filepaths[1], function (err) {
                            //do not stop execution of the loop on error since failure to clean
                            //the tmp files is not critical to this operation
                            if (err)
                                module.exports.log("error", "Error occurred when cleaning the tmp path:", err.stack);
                            next();
                        });
                    }, 
                    ...
                    function finalCallback() {
                        fs.rmdir(config.tmpDir + "/" + targetName, function (err) {
                            //if the empty dir fails to be deleted we must undo the renamings
                            if (err)
                                module.exports.log("error", "Error occurred when cleaning the leftover tmp dir:", err.stack);
                            resolve();
                        });
                    });
                });
    ...
    function undo(err, cb) {
        ...
        fs.rmdir(config.tmpDir + "/" + targetName, function (rmdirErr) {
            //if the empty dir fails to be deleted we must undo the renamings
            if (rmdirErr)
                module.exports.log("error", "Error occurred when cleaning the leftover tmp dir:", rmdirErr.message);
            cb(err);
        });
    }
};
...
module.exports.deleteDir = function (path, cb) {
    return new Promise(function (resolve, reject) {
        fs.readdir(path, function (err, filenames) {
            ...
            filenames.forEachPromise(function (filename, index, next) {
                //delete contained dir through recursion as well
                if (fs.lstatSync(path + "/" + filename).isDirectory() === true) {
                    module.exports.deleteDir(path + "/" + filename, function () {
                        if (err) {
                            if (typeof cb === "function")
                                return cb(err);
                            else
...
module.exports.readJSON = function (path, reviver, cb) {
    ...
    return new Promise(function (resolve, reject) {
        fs.readFile(path, "utf8", function (err, data) {
            if (err) {
                var myErr = new Error("There was an error while trying to read the JSON file " + path + ":\n\n" + err.message);
            ...
            if (/[\w\d]/.test(data) === false) {
                var myErr = new Error("No data in " + path + ".");
...
module.exports.saveJSON = function (path, obj, cb) {
    return new Promise(function (resolve, reject) {
        fs.writeFile(path, module.exports.JSONStringify(obj), function (err) {
            if (err) {
...
exports.getDirSubfolderNamesSync = function (path) {
    var filenames = exports.getAllDirFilenamesSync(path);
    var subfolderNames = [];
    filenames.forEach(function (filename) {
        var stat = fs.statSync(path + "/" + filename);
...
exports.readDirContentsSync = function (path) {
...
exports.fetchAllDirFilenames = function (path) {
    return Promise.resolve()
        .then(function () { return fs.readdir(path, "utf8", function (error, filenames) {
        if (error)
...
exports.fetchOnlyDirFilenames = function (path) {
    var onlyFilenames = [];
...
exports.fetchDirSubfolderNames = function (path) {
    var subfolderNames = [];
    return exports.fetchAllDirFilenames(path)
        .then(function (filenames) {
        filenames.forEach(function (filename) {
...
exports.readDirContents = function (path) {
    ...
    return exports.fetchOnlyDirFilenames(path)
        .then(function (filenames) {
        return filenames.forEachPromise(function (filename, index, nextIteration) {
        ...
        .then(function () { return Promise.resolve(contents); });
};
module.exports.log = function (tags, trace) {
    var inputs = [];
    for (var _i = 2; _i < arguments.length; _i++) {
        inputs[_i - 2] = arguments[_i];
    }
    ...
    tags.forEachPromise(function (tag, index, next) {
        var path = config.pathToLogs + "/" + tag + ".txt";
        var writeFn = (fs.existsSync(path)) ? fs.appendFile : fs.writeFile;
...
module.exports.JSONStringify = function (obj, spacing) {
...
//# sourceMappingURL=reader_writer.js.map